...
final class AddExpenseViewController: UIViewController {
    ...
    @IBOutlet weak var noteTextField: UIView!
    
    @IBOutlet weak var categoryLabel: UILabel!
    
    @IBOutlet weak var tagLabel: UILabel!
    
    private var cateogory: Category?
    
    convenience init(cateogory: Category) {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        if let cateogory = cateogory {
            setupView(with: cateogory)
        }
    }
    
    private func setupView(with category: Category) {
        categoryLabel.text = cateogory?.title
        tagLabel.text = cateogory?.tag?.title
    }
}
